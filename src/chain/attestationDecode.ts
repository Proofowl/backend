 ...
 * (crate 0.3.0) contract. The bug is in
 * `@stellar/stellar-sdk`'s spec-driven struct decoder (`Spec.structToNative`,
 * pinned `^16.0.1` by the SDK) — it mis-aligns the key-sorted `ScMap`
 * the host returns against the embedded contract spec's field order.
 * The scalar reads (`get_admin`, `get_attestor`, `get_attestation_count`,
 * `get_reputation_score`, `get_wallet_for_github`, `get_github_for_wallet`)
 * are unaffected and are used directly from the SDK. The `stellar` CLI
 * 28.0.0 decodes the same call correctly.
 *
 ...
 * release that decodes the struct, and switch the callers back to
